...
for path in pathlib.Path("dsa_test/pdf_files").iterdir():
    if path.is_file():
        ...
        for i in range(len(list_of_courses) - 1):
            course_edges.append((list_of_courses[i], list_of_courses[i+1]))
        ...
        df = pd.DataFrame(course_edges, columns =['CourseOne', 'CourseTwo' ])
        df.to_csv(f'dsa_test/csv_files/{path.name}.csv')
        current_file.close()